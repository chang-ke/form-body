(function () {
  ...
  function defaultRule(filename, index) {
    ...
    const md5 = crypto.createHash("md5").update(timestamp + filename + index).digest("hex");
    return md5;
  }
   ...
   * @returns {promise}
   */
  function formBody(options) {
    if (typeof options.ctx === 'undefined' || typeof options.path === 'undefined') {
      const ctx = options.ctx === undefined ? 'ctx' : '';
      const path = options.path === undefined ? 'path' : '';
      throw new Error(`${ctx} ${path} is required`);
      return new Promise((resolve, reject) => {
        reject(`${ctx} ${path} is required`);
      });
    }
